...
import copy
import json
from get_data.settings import dividech
...
if __name__=='__main__':
    # 找到JsonPath-----------------------------------------------------------------------------------------------------------------
    # dic1=""
    # with open('json.txt','r',encoding='utf-8') as wfile:
    #     dic1=wfile.read()
    #
    ...
    #
    # # 要检索的内容
    # in_value_path = a.in_value_path('深圳市市场和质量监督管理委员会南山局')  # 包含匹配，只要dict/list的元素中包含这个str，就能得到对应的path
    #
    ...
    #
    #
    # print(dic1['datalist']['taxeditlist']['data'])


    # # 格式化-----------------------------------------------------------------------------------------------------------------
    f = open('credit.txt', 'w')
    cmpList=[]
    with open('..\get_data\data.txt','r') as rfile:
        cmpList=rfile.read()

    cmpList=cmpList.split(dividech)
    ...
    print('json出错：{}\t无相关信息{}'.format(erroNum,noInfNum))
    f.close()
